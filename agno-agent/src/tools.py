...
Returns async tool functions that share:
  - queue:      asyncio.Queue → pushes SSE events upstream
  - code_state: dict          → { code, library } mutable shared state
"""
...
def make_tools(queue: asyncio.Queue, code_state: dict) -> list:
    ...
    async def commit_code(code: str, library: str, description: str) -> str:
        ...
        Pass the full code in the `code` parameter.
        library must be one of: gsap, anime, pixi, three, canvas.
        YOU MUST call validate_code() immediately after. No exceptions.
    ...
    async def str_replace(old_str: str, new_str: str, description: str) -> str:
        ...
        old_str must be unique in the code.
        YOU MUST call validate_code() immediately after.
    ...
    async def validate_code() -> str:
        ...
        Fix all warnings before summarizing.
        ...
        if result["ok"] and not result["warnings"]:
            lines.append("All checks passed. You may now output '✓ done'.")
        elif result["ok"] and result["warnings"]:
            lines.append("ok=true but warnings exist. Fix with str_replace, then call validate_code again.")
